...
- If Ark returns `NoAvailableModel`, the request reached Volcengine Ark but the configured inference endpoint has no available online model instance. Tell the user to check the `ep-*` endpoint status, region, model binding, and quota/instances in the Ark console. If the endpoint is healthy but still unavailable, enable it again in the console or call `StartEndpoint`; `GetEndpoint` on `open.volcengineapi.com` with `Action=GetEndpoint`, `Version=2024-01-01`, and `Id=<ep-id>` can inspect `Status` and `ModelReference`.
